...
func TestLoginAPI(t *testing.T) {
	...
	_, err := db.Exec(`
        ...
        VALUES ('player123', 'testuser', ?, 'Test User', 100.0)
    `, hashedPassword)
	if err != nil {
	...
	log.Printf("Response code: %d, body: %s", resp.Code, resp.Body.String())

	// Assert the response
	assert.Equal(t, http.StatusOK, resp.Code, "Expected HTTP 200 OK")
	var response map[string]interface{}
	json.Unmarshal(resp.Body.Bytes(), &response)
	assert.Contains(t, response, "player_id", "Response should contain player_id")
	assert.Contains(t, response, "balance", "Response should contain balance")
}
